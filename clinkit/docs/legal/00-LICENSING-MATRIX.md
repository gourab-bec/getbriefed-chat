# Licensing & Registration Matrix — Goods-Delivery Marketplace (GetBriefed Inc/LLC d/b/a Clinkit)
...
| Item | Status / action |
|---|---|
| EIN | Assumed held by GetBriefed. Add d/b/a "Clinkit" via county/state fictitious-name filing. |
...
| Fictitious Business Name ("Clinkit") | County (San Joaquin) | ~$50 + newspaper publication. |
| **Prop 22 (app-based driver law)** | — | Clinkit is likely a "delivery network company" under Prop 22. Compliance duties: 120% of local min-wage earnings floor on engaged time + $0.35/mi (indexed), healthcare stipend at 15+/25+ engaged hrs/wk, occupational-accident insurance ($1M), anti-discrimination policy, no mandatory schedules/exclusivity. **Earnings-floor top-up logic must be added to payout engine before scale** (tracked in ops runbook; MVP volume makes exposure small but non-zero). |
